								...
								@foreach ($data as $item)
			                        @if (empty($item->parent_id))
			                            <li class="dd-item" data-id="{{ $item->id }}">
			                                <div class="dd-handle">
			                                    <p>{{ $item->title }} (<i>{{ $item->url }}</i>)</p>
												<p>{{ $item->title_en }} (<i>{{ $item->url_en }}</i>)</p>
			                                </div>
			            ...
			            <div class="modal-content">
			                ...
			                <form action="{{ route('setting.menu.addItem', $id ) }}" method="POST" class="frm_add">
			                    <div class="modal-body">
			                        ...
			                        <fieldset class="form-group">
			                            <label>Đường đẫn tiếng việt</label><br>
									...
									<fieldset class="form-group">
			                            ...
			                            <input type="text" class="form-control" placeholder="Nhập tiêu đề" name="title_en" required>
									</fieldset>
			                        <fieldset class="form-group">
			                            <label>Đường đẫn tiếng anh</label><br>
			                            <label>Chỉ coppy phần bôi đỏ: <br>
			                                {{ url('/') }}<span style="color: red; font-weight: bold;">/introduce</span>
			                            </label>
			                            <div class="input-group">
			                                <span class="input-group-addon">{{ url('/') }}</span>
			                                <input type="text" class="form-control" placeholder="Nhập đường dẫn" name="url_en" required>
			                            </div>
			                        </fieldset>
			                        <fieldset class="form-group">
			            ...
			            <div class="modal-content">
			                ...
			                <form action="{{ route('setting.menu.editItem' ) }}" method="POST" class="frm_add">
			                    <div class="modal-body">
			                        ...
			                        <fieldset class="form-group">
			                            <label>Đường đẫn tiếng việt</label>
			                            <input type="text" class="form-control" id="editUrl" name="url" required>
			                        </fieldset>
									<fieldset class="form-group">
			                            <label>Tiêu đề tiếng anh</label>
			                            <input type="text" class="form-control" id="editTitleEn" name="title_en" required >
									</fieldset>
			                        <fieldset class="form-group">
			                            <label>Đường đẫn tiếng anh</label>
			                            <input type="text" class="form-control" id="editUrlEn" name="url_en" required>
			                        </fieldset>
			                    </div>
			                    <div class="modal-footer">
        ...
        $('.modalEditMenu').click(function(event) {
            ...
            $.get('{{ asset('/backend/menu/edit-item/') }}/'+id, function(data) {
                if(data.status == "success"){
                    ...
                    $('#editUrlEn').val(data.data.url_en);
                    $('#id_menu').val(id);
                    $('#editMenu').modal('show')
                }
            });
